...
usage() {
  cat <<'EOF'
Usage:
  install-solana-blinks.sh --base-url https://HOST --recipient SOLANA_PUBLIC_KEY --usdc-escrow-recipient SOLANA_PUBLIC_KEY [--rpc-url HTTPS_URL]
...
while (($#)); do
  case "$1" in
    ...
    --usdc-escrow-recipient) USDC_ESCROW_RECIPIENT="${2:-}"; shift 2 ;;
...
if [[ ! "$USDC_ESCROW_RECIPIENT" =~ ^[1-9A-HJ-NP-Za-km-z]{32,44}$ ]]; then
  echo "--usdc-escrow-recipient does not look like a Solana public key" >&2
  exit 2
fi
...
FIFA_BLINK_URL="$(node -e 'const endpoint=process.argv[1]+"/api/actions/fifa26"; console.log("https://dial.to/?action="+encodeURIComponent("solana-action:"+endpoint))' "$BASE_URL")"
HELLO_BLINK_URL="$BASE_URL/blinks/hello-solana"
...
sed -i "s|__SOLANA_FIFA26_BLINK_URL__|$FIFA_BLINK_URL|g" "$WORKSPACE/.openclaw/skills/solana-blinks/SKILL.md"
sed -i "s|__SOLANA_HELLO_BLINK_URL__|$HELLO_BLINK_URL|g" "$WORKSPACE/.openclaw/skills/solana-blinks/SKILL.md"

cat > "$DROPIN/solana-blinks.conf" <<EOF
[Service]
Environment="SOLANA_BLINK_BASE_URL=$BASE_URL"
Environment="SOLANA_BLINK_RECIPIENT=$RECIPIENT"
Environment="SOLANA_USDC_ESCROW_RECIPIENT=$USDC_ESCROW_RECIPIENT"
Environment="SOLANA_USDC_MINT=EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v"
...
curl -fsS "$BASE_URL/api/actions/fifa26" >/dev/null
curl -fsS "$BASE_URL/api/actions/hello-solana" >/dev/null
openclaw health

echo
echo "Solana Blink installed:"
echo "$BLINK_URL"
echo "FIFA 26 USDC Blink:"
echo "$FIFA_BLINK_URL"
echo "Hello Solana test Blink:"
echo "$HELLO_BLINK_URL"
echo
echo "Telegram test: message the bot 'hello Solana'."
