...
function actionUrl(amount) {
    if (amount === "{amount}")
        return `${baseUrl()}${ACTION_PATH}?amount={amount}`;
    const url = new URL(`${baseUrl()}${ACTION_PATH}`);
    if (amount)
        url.searchParams.set("amount", amount);
    return url.toString();
}
function fifaActionUrl(requestId) {
    const url = new URL(`${baseUrl()}${FIFA_ACTION_PATH}`);
    if (requestId)
        url.searchParams.set("request", requestId);
    return url.toString();
}
function helloActionUrl(requestId) {
...
function validRequestId(value) {
    if (!value || !/^[0-9a-f-]{36}$/i.test(value))
        throw new Error("invalid FIFA 26 request reference");
    return value;
}
...
function fifaMetadata(requestId = crypto.randomUUID()) {
    return {
        type: "action",
        icon: `${baseUrl()}${ICON_PATH}`,
...
function helloMetadata(requestId = crypto.randomUUID()) {
    return {
        type: "action",
        icon: `${baseUrl()}${ICON_PATH}`,
        title: "Hello Solana",
...
async function createTransaction(account, amountRaw) {
...
function buildFifaTransaction(sender, escrowOwner, mint, requestId, blockhash, lastValidBlockHeight) {
    ...
    return new Transaction({ feePayer: sender, blockhash, lastValidBlockHeight }).add(createAssociatedTokenAccountIdempotentInstruction(sender, escrowTokenAccount, escrowOwner, mint), createTransferCheckedInstruction(senderTokenAccount, mint, escrowTokenAccount, sender, BigInt(FIFA_PRICE_BASE_UNITS), USDC_DECIMALS), new TransactionInstruction({ programId: MEMO_PROGRAM_ID, keys: [], data: Buffer.from(memo, "utf8") }));
}
function buildHelloTransaction(sender, requestId, blockhash, lastValidBlockHeight) {
    return new Transaction({ feePayer: sender, blockhash, lastValidBlockHeight }).add(new TransactionInstruction({
        programId: MEMO_PROGRAM_ID,
        keys: [],
        data: Buffer.from(`${HELLO_MEMO_PREFIX}${requestId}`, "utf8"),
    }));
}
async function createHelloTransaction(account, requestId) {
    const sender = new PublicKey(account);
    const rpcUrl = process.env.SOLANA_RPC_URL?.trim() || "https://api.mainnet-beta.solana.com";
    const connection = new Connection(rpcUrl, "confirmed");
    const { blockhash, lastValidBlockHeight } = await connection.getLatestBlockhash("confirmed");
...
async function createFifaTransaction(account, requestId) {
    const sender = new PublicKey(account);
...
async function verifyFifaPayment(signature, expectedRequest) {
    ...
    if (!memo.startsWith(FIFA_MEMO_PREFIX))
        throw new Error("transaction is not a FIFA 26 payment");
    const requestId = validRequestId(memo.slice(FIFA_MEMO_PREFIX.length));
    if (expectedRequest && requestId !== expectedRequest)
        throw new Error("payment reference does not match this request");
    const escrowOwner = new PublicKey(requiredEnv("SOLANA_USDC_ESCROW_RECIPIENT"));
    const mint = new PublicKey(process.env.SOLANA_USDC_MINT?.trim() || MAINNET_USDC_MINT);
    ...
    if (escrowIndex < 0)
        throw new Error("escrow token account is missing from transaction");
    const balance = (items) => {
    ...
    if (received !== BigInt(FIFA_PRICE_BASE_UNITS))
        throw new Error("transaction did not deposit exactly 0.01 USDC");
...
function recordReceipt(payment, consume) {
    const receipts = readReceipts();
    const existing = receipts[payment.signature];
    if (consume && existing?.status === "consumed")
        throw new Error("this FIFA 26 payment was already used");
...
function web3BrowserBundle(res) {
    try {
...
async function fifaActionHandler(req, res) {
    try {
        if (req.method === "OPTIONS") {
            cors(res);
            res.statusCode = 204;
        ...
        json(res, 400, { message: error?.message || "Unable to create FIFA 26 payment" });
        return true;
    }
}
async function fifaCallbackHandler(req, res) {
    try {
        if (req.method === "OPTIONS") {
            cors(res);
            res.statusCode = 204;
        ...
        json(res, 400, { message: error?.message || "Unable to verify FIFA 26 payment" });
        return true;
    }
}
async function fifaVerifyHandler(req, res) {
    try {
        if (req.method !== "GET") {
            json(res, 405, { message: "Method not allowed" });
            return true;
        ...
        json(res, 400, { verified: false, message: error?.message || "Unable to verify FIFA 26 payment" });
        return true;
    }
}
async function helloActionHandler(req, res) {
    try {
        if (req.method === "OPTIONS") {
            cors(res);
            res.statusCode = 204;
        ...
        json(res, 400, { message: error?.message || "Unable to create Hello Solana transaction" });
        return true;
    }
}
async function helloCallbackHandler(req, res) {
    try {
        if (req.method === "OPTIONS") {
            cors(res);
            res.statusCode = 204;
        ...
        json(res, 400, { message: error?.message || "Unable to verify Hello Solana transaction" });
        return true;
    }
}
export default function register(api) {
    api.registerHttpRoute({ path: "/actions.json", auth: "plugin", match: "exact", handler: async (_req, res) => {
            json(res, 200, { rules: [
    ...
    api.registerHttpRoute({ path: ICON_PATH, auth: "plugin", match: "exact", handler: async (_req, res) => {
            icon(res);
            return true;
        } });
    ...
    api.registerHttpRoute({ path: HELLO_PAGE_PATH, auth: "plugin", match: "exact", handler: async (_req, res) => {
            helloPage(res);
            return true;
        } });
    api.registerHttpRoute({ path: HELLO_CLIENT_PATH, auth: "plugin", match: "exact", handler: async (_req, res) => {
            helloClient(res);
            return true;
        } });
    api.registerHttpRoute({ path: WEB3_BROWSER_PATH, auth: "plugin", match: "exact", handler: async (_req, res) => {
            web3BrowserBundle(res);
            return true;
        } });
}
